...
\begin{figure}[htb]
  \centering
  \subfloat{\includegraphics[width=0.4\textwidth]{../graphics/Vis_tsne.pdf}} \hspace{1cm}
  \subfloat{\includegraphics[width=0.4\textwidth]{../graphics/Vis_tsne2.pdf}} 
\end{figure}
\end{frame}
...
\begin{frame}{Saliency maps}
	...
	\end{figure}
	For each pixel in the input image we can calculate the derivative of the loss with respect to that pixel value \cite{Simonyan:2013}. 
	This allows us to visualize how much the output changes with changes in the pixel values.
\end{frame}
...
\end{figure}
With this, we can see which pixels contribute to the classification result. 
Saliency maps can also be used for unsupervised segmentation.
\end{frame}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item For visualization, we require that the directional values in $d$ corresponding to filter parameters $(r,l)$ have the same norm as the filter values. 
		\begin{equation}
